   ...
   _oDlgArray := MSDialog ():New (_nNivel * 25 - 25, ;  // Superior
      ...
      elseif len (_aMatOrig) == 0
         ...
         _oBrowse := TWBrowse ():New (5, ;  // Superior
                                      5, ;  // Esquerda
                                      _oDlgArray:nClientWidth / 2 - 5, ;  // Largura
                                      _oDlgArray:nClientHeight / 2 - 45, ;  // Altura
         ...
         _oBrowse:oFont := TFont():New ("Courier New", 06, 20)
         ...
         // Informacoes gerais
         @ (_oDlgArray:nClientHeight / 2 - 35), 10 say "Dimensao da matriz: " + alltrim (str (_oBrowse:nLen)) + " x " + alltrim (str (iif (valtype (_aMatOrig [1]) != "A", 0, len (_aMatOrig [1]))))
         @ (_oDlgArray:nClientHeight / 2 - 25), 10 say "Linha atual:"
         @ (_oDlgArray:nClientHeight / 2 - 25), 40 get _oBrowse:nAT picture "9999" when .F. object _oLinAtu
      endif
      @ (_oDlgArray:nClientHeight / 2 - 30), (_oDlgArray:nClientWidth / 2 - 35) bmpbutton type 1 action (_oDlgArray:End ())
   _oDlgArray:Activate (, ;     // ?
                        , ;     // ?
                        , ;     // ?
                        .F., ;  // Centralizado
