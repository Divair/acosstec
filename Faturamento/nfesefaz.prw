...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	If At("<gDevTrib>", cString) > 0
		ConOut("[GDEVTRIB] NFESEFAZ custom ativo | Antes:S")
		cString := AjGDevZr(cString)
		ConOut("[GDEVTRIB] NFESEFAZ custom ativo | Depois:" + ;
			IIf(At("<gDevTrib>", cString) > 0, "S", "N"))
	EndIf
...
/*/{Protheus.doc} AjGDevZr
Workaround temporario para a release 12.1.2410.
Remove somente o grupo gDevTrib cujo vDevTrib esteja zerado.
Grupos com valor diferente de zero sao preservados.
@type function
@since 03/08/2026
@param cXmlNfe, character, XML da NF-e antes da conversao UTF-8
@return character, XML ajustado
/*/
Static Function AjGDevZr(cXmlNfe)
	Local cXmlRetorno	:= ""
	Local cXmlRestante	:= cXmlNfe
	Local cTagGrupoIni	:= "<gDevTrib>"
	Local cTagGrupoFim	:= "</gDevTrib>"
	Local cTagValorIni	:= "<vDevTrib>"
	Local cTagValorFim	:= "</vDevTrib>"
	Local cGrupoDev		:= ""
	Local cValorDev		:= ""
	Local nPosGrupoIni	:= 0
	Local nPosGrupoFim	:= 0
	Local nPosValorIni	:= 0
	Local nPosValorFim	:= 0
	Local nTamanhoGrupo	:= 0
	Local lRemoveGrupo	:= .F.

	Default cXmlNfe := ""

	nPosGrupoIni := At(cTagGrupoIni, cXmlRestante)
	While nPosGrupoIni > 0
		cXmlRetorno += Left(cXmlRestante, nPosGrupoIni - 1)
		cXmlRestante := SubStr(cXmlRestante, nPosGrupoIni)

		nPosGrupoFim := At(cTagGrupoFim, cXmlRestante)
		If nPosGrupoFim == 0
			cXmlRetorno += cXmlRestante
			cXmlRestante := ""
			Exit
		EndIf

		nTamanhoGrupo := nPosGrupoFim + Len(cTagGrupoFim) - 1
		cGrupoDev := Left(cXmlRestante, nTamanhoGrupo)
		cValorDev := ""
		lRemoveGrupo := .F.
		nPosValorIni := At(cTagValorIni, cGrupoDev)
		nPosValorFim := At(cTagValorFim, cGrupoDev)

		If nPosValorIni > 0 .And. nPosValorFim > nPosValorIni
			cValorDev := SubStr(cGrupoDev, ;
				nPosValorIni + Len(cTagValorIni), ;
				nPosValorFim - (nPosValorIni + Len(cTagValorIni)))

			lRemoveGrupo := !Empty(AllTrim(cValorDev)) .And. ;
				Val(StrTran(AllTrim(cValorDev), ",", ".")) == 0
		EndIf

		If !lRemoveGrupo
			cXmlRetorno += cGrupoDev
		EndIf

		cXmlRestante := SubStr(cXmlRestante, nTamanhoGrupo + 1)
		nPosGrupoIni := At(cTagGrupoIni, cXmlRestante)
	EndDo
